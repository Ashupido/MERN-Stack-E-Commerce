...
export default function SellerDashboard({ addToast, refreshTrigger = 0 }) { // Added refreshTrigger prop
  ...
  useEffect(() => {
    const fetchProducts = async () => {
      try {
        setLoading(true);
       const res = await API.get('/products?limit=1000'); // Request a large number for the seller
    ...
    fetchProducts(); // Call fetchProducts when component mounts or refreshTrigger changes
  }, [addToast, refreshTrigger]); // Added refreshTrigger to dependency array
